...
const UNSPLASH_ACCESS_KEY = 'OdzvanAPXMDpuYyx1vl_Njl4IQHhYIK2j4bx7CzxDuU';
...
export default function GeneratePlan() {
  ...
  // Function to fetch Unsplash images for places
  ...
  // ---------- Helpers (DEFINED BEFORE HANDLERS) ----------
  ...
  useEffect(() => {
    ...
    if (itineraryDays[activeDay]?.places) {
      loadImages();
    }
  ...
  // FIXED SAVE FUNCTION - Now with itineraryDays properly defined above
  const handleSave = async () => {
    try {
      ...
      showToast("✅ Trip saved successfully! View in My Trips");
      
      // Ask user if they want to view saved trips
      setTimeout(() => {
        if (window.confirm('Trip saved! Would you like to view your saved trips?')) {
    ...
    <div className="generate-plan-container" style={{ 
      ...
      >
        ...
        <div className="content-left" style={{ flex: 2, minWidth: "50%" }}>
          ...
          {/* ACTIVITY PLACES - WITH UPDATED IMAGES */}
          <div className="timeline-card" style={{
            ...
            {/* Place cards with Unsplash images */}
            <div className="places-container" style={{ display: 'grid', gap: '20px' }}>
              {itineraryDays[activeDay]?.places?.map((place, idx) => (
                <div key={idx} className="place-card" style={{
                  ...
                  <div className="place-image" style={{ width: '200px', height: '150px', position: 'relative' }}>
                    <img
                      src={placeImages[place.name] || `https://source.unsplash.com/400x300/?${encodeURIComponent(place.name)}`}
                      alt={place.name}
